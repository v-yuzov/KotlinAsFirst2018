...
fun ageDescription(age: Int): String {
    ...
    if (age % 100 > 4 && age % 100 < 20) res += " лет"
    else if (age % 10 == 1) res += " год"
    else if (age % 10 < 5) res += " года"
    else res += " лет"
    return res;
}
...
fun timeForHalfWay(t1: Double, v1: Double,
    ...
    if (halfWay <= t1 * v1) res = halfWay / v1
    else if (halfWay <= t1 * v1 + t2 * v2) res = t1 + (halfWay - t1 * v1) / v2
    else res = t1 + t2 + (halfWay - t1 * v1 - t2 * v2) / v3
...
fun triangleKind(a: Double, b: Double, c: Double): Int {
    ...
    x.sort()
    if (x[2] > x[0] + x[1]) return -1
    if (x[2] * x[2] < x[0] * x[0] + x[1] * x[1]) return 0
    if (x[2] * x[2] == x[0] * x[0] + x[1] * x[1]) return 1
    return 2
}
